...
class Preproc(object):
    ...
    def __init__(self, num_orients=16, filter_scale=4.0, cross_channel_pooling=False):
    ...
    def fwd_infer(self, img, brightness_diff_threshold=40.0):
...
def generate_suppression_masks(filter_scale=4.0, num_orients=16):
    """
    ...
    for i, angle in enumerate(
        np.linspace(0.0, np.pi, num_orients // 2, endpoint=False)
    ):
